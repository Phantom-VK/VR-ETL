...
- Generic LLM access: set `API_KEY`, `BASE_URL`, `CHAT_MODEL` (used for answer, default temp 0.2), `REASONING_MODEL` (used for search, default temp 0.1). Requests can override `search_model`, `answer_model`, and `temperature` (single override applied to both temps if set).
